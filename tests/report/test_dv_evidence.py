...
def test_a_flow_forwards_completions_to_its_collector() -> None:
    """`SimCfg.on_job_completed` is the override the scheduler drives, so it has to forward.

    Called unbound against a stand-in, since constructing a real `SimCfg` needs a whole hjson cfg
    and none of it bears on the forwarding.
    """
    flow = SimpleNamespace(run_evidence=RunEvidenceCollector())

    SimCfg.on_job_completed(flow, _spec("hmac_smoke", seed=3), JobStatus.PASSED, None)
