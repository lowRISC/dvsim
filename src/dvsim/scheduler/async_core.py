...
class Scheduler:
    ...
    def _build_graph(self, specs: Iterable[JobSpec]) -> None:
        ...
        for spec in specs:
            if spec.id in self._jobs:
                ...
                continue
            if spec.backend is not None and spec.backend not in self.backends:
                err = f"Unknown job backend '{spec.backend}'"
                raise ValueError(err)
            backend_name = self.default_backend if spec.backend is None else spec.backend
            self._jobs[spec.id] = JobRecord(spec=spec, backend_key=backend_name)
        ...
        for job in self._jobs.values():
            ...
            for dep in job.spec.dependencies:
                if dep not in self._jobs:
                    ...
                    raise ValueError(err)
                self._jobs[dep].dependents.append(job.spec.id)
    ...
    def _validate_acyclic(self) -> None:
        ...
        indegree = {job: record.remaining_deps for job, record in self._jobs.items()}
        ...
        while job_queue:
            job = job_queue.pop()
            num_visited += 1
            for dep in self._jobs[job].dependents:
                indegree[dep] -= 1
                if indegree[dep] == 0:
                    job_queue.append(dep)

        if num_visited != len(self._jobs):
    ...
    async def _schedule_ready_jobs(self) -> None:
        ...
        available_slots = (
            self.max_parallelism - len(self._running)
        ...
        while self._ready_heap and slots_used < available_slots:
            ...
            job = self._jobs[job_id]
            backend = self.backends[job.backend_key]
